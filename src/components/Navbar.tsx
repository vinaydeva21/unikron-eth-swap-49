    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button 
            className={`bg-unikron-blue hover:bg-unikron-blue-light text-white transition-all duration-300 ${isMobile ? 'px-3 py-1 text-sm' : ''}`}
          >
            <Wallet className={`${isMobile ? 'mr-1 h-3 w-3' : 'mr-2 h-4 w-4'}`} />
            {isMobile 
              ? (connected ? "Wallet" : "Connect") 
              : (connected ? selectedWallet?.name || "Connected" : "Connect Wallet")}
          </Button>
        </DialogTrigger>
        <DialogContent className="bg-unikron-navy-light border border-unikron-blue/20 text-white max-w-[90vw] sm:max-w-md">
          <DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3 p-4">
            {WALLET_PROVIDERS.map((wallet) => (
              <button
                key={wallet.id}
                className="flex flex-col items-center justify-center p-3 rounded-xl border border-unikron-blue/20 hover:bg-unikron-blue/10 transition-all duration-300"
              ...
              >
                ...
                />
                <span className="text-xs sm:text-sm text-center whitespace-nowrap overflow-hidden text-ellipsis w-full">
                  {wallet.name}
                </span>
              </button>
            ))}
